...
def capture(storage,now=None):
 ...
 hours={};days=set(daily)|set(extras)
 for row in sessions:
 ...
 while d<=now.date():
  key=d.isoformat();base=daily.get(key,{});extra=extras.get(key,{});stages=extra.get('sleep_stages') or {};totals={k:sum(hours.get((key,h,k),0) for h in range(24)) for k in ('work','learning')}
  ...
  for h in range(24):
   ...
   hourly.append({'date':key,'hour_start_local':f'{key}T{h:02}:00:00','work_seconds':int(hours.get((key,h,'work'),0)),'learning_seconds':int(hours.get((key,h,'learning'),0))})
  d+=timedelta(days=1)
 ...
 return {'captured_at':now.isoformat(timespec='seconds'),'daily':rows,'hourly':hourly,'sessions':sessions,'health':snapshots}
